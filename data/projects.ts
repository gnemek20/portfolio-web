...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tags: ["Next.js 14", "FastAPI", "Supabase", "WebSocket", "DooD", "pgvector"],
    highlights: [
      "매니페스트 기반 동적 UI — inputs[] / view_type 만 보고 폼·결과 렌더러 자동 생성 (신규 에이전트 등록 시 프론트 코드 수정 0줄)",
      "ZIP 업로드 → precheck 자동 실행 → haire.yaml 자동 생성 → JSON 에디터 편집의 4단계 단축 흐름",
      "WebSocket 5-이벤트 프로토콜 UI (output / input_request / result / error / done)",
      "SessionStorage JWT + Google OAuth + 에이전트별 OAuth(Google/GitHub) 인증 플로우",
      "KaTeX + rehype-sanitize 마크다운 렌더링 (수식 지원 + XSS 방지)",
      "FIELD_MAP / RENDERERS 매핑으로 새 input.type · view_type 1줄 추가 확장 구조",
      "에이전트/채팅 공유 페이지(share) + Agent Manifest Guide 문서 체계",
      "팀원의 DooD 백엔드 + pgvector 시맨틱 검색을 클라이언트 측 인터페이스로 통합",
    ],
    architecture:
      "front/ → Next.js 14 (Pages Router) + Dynamic UI (FIELD_MAP / RENDERERS) + WebSocket + UserContext\nback/ → FastAPI + Docker SDK (DooD) + Supabase (PostgreSQL + pgvector) + Agent Runner",
    accordions: [
      {
        title: "이용 흐름",
        blocks: [
          ...
          {
            type: "text",
            value: "에이전트 상세 페이지에서는 소개·사용 중인 AI·필요한 인증·입력 항목을 확인하고 구독 후 사용할 수 있습니다. 입력 폼은 매니페스트의 `inputs[]` 만 보고 자동 생성되며, 결과 화면 또한 `outputs.view_type` 에 따라 markdown / json / json_table 등 적절한 렌더러로 자동 전환됩니다.",
          },
          {
            type: "image",
          ...
          {
            type: "text",
            value: "에이전트는 WebSocket 으로 실시간 실행됩니다. 5종 이벤트 — `output` (스트리밍 텍스트) · `input_request` (추가 입력 요청) · `result` (최종 결과) · `error` · `done` — 를 클라이언트가 처리하며, 응답은 KaTeX 수식과 rehype-sanitize 기반 안전한 마크다운으로 렌더링됩니다.",
          },
          {
            type: "image",
          ...
          {
            type: "text",
            value: "개발자 흐름은 Share Agent 페이지로 통합했습니다. ZIP 파일을 업로드하면 precheck 가 자동 실행되고, AI 가 코드를 분석해 `haire.yaml` 매니페스트를 자동 생성합니다. 기존의 \"ZIP 업로드 → 버튼 클릭 → 매니페스트 생성\" 흐름을, ZIP 업로드 한 번으로 매니페스트까지 한 번에 도달하도록 단순화한 결과입니다.",
          },
          {
            type: "image",
          ...
          {
            type: "text",
            value: "생성된 매니페스트는 직관적인 에디터에서 JSON 형태로 다듬을 수 있습니다. raw YAML 을 직접 손볼 필요 없이, 입력 항목·출력 형식·인증 요구사항 등을 폼 단위로 편집하면서 매니페스트가 그대로 갱신됩니다. \"누구나 쉽게 에이전트를 사고 팔 수 있다\" 라는 Haire 의 정체성을 살린 설계입니다.",
          },
          {
            type: "image",
      ...
      },
      {
        ...
        blocks: [
          {
            type: "text",
            value: "Haire 는 hAIre_Back(유저 인증·구독·OAuth)과 haire_backend(에이전트 CRUD·Docker 빌드·WebSocket 실행·시맨틱 검색) 두 개의 백엔드로 운영됩니다. haire_backend 는 DooD(Docker-outside-of-Docker) 아키텍처로, 플랫폼 컨테이너가 호스트의 `/var/run/docker.sock` 을 마운트해 에이전트 컨테이너를 형제로 생성합니다. 별도 Docker 데몬 없이 호스트 이미지 캐시를 공유하므로 빠르고 가볍습니다.",
          },
          {
            type: "text",
            value: "에이전트 배포 파이프라인은 zip 추출 → `haire.yaml` 파싱 → Dockerfile 자동 생성 → `agent_runner.py` 주입 → `docker build` → OpenAI `text-embedding-3-small` 임베딩 → Supabase upsert 까지 한 번에 처리됩니다. 실행 시점에는 컨테이너가 `--rm` 플래그로 즉시 삭제되며, stdout 의 `__AGENT_RESULT__` / `__INPUT_REQUEST__` 마커 프로토콜로 결과·추가 입력 요청을 주고받습니다.",
          },
          {
            type: "text",
            value: "프론트엔드의 핵심은 매니페스트 기반 동적 UI 입니다. `types/manifest.ts` 를 단일 진실(source of truth)로 두고, `components/dynamic/DynamicInputForm.tsx` 는 `inputs[]` 를 보고 `FIELD_MAP` 에서 적합한 필드 컴포넌트(text / number / select / boolean / file …)를 골라 폼을 그리며, `DynamicOutputView.tsx` 는 `outputs.view_type` 을 보고 `RENDERERS` 매핑으로 markdown / json / json_table 등 렌더러를 선택합니다. 알 수 없는 타입은 안전한 폴백으로 처리되어 신규 에이전트가 들어와도 화면이 깨지지 않습니다.",
          },
          {
            type: "text",
            value: "새로운 입력 타입이나 출력 view_type 을 추가하려면 — fields.tsx / renderers.tsx 에 컴포넌트를 작성하고, FIELD_MAP / RENDERERS 매핑에 한 줄, 매니페스트 타입 유니온에 리터럴 한 개만 추가하면 끝입니다. 페이지 코드는 어디도 수정할 필요가 없습니다.",
          },
          {
            type: "text",
            value: "채팅 화면은 5종 WebSocket 이벤트(output / input_request / result / error / done)를 처리하는 전용 훅 `useAgentRun.ts` 으로 추상화했습니다. `output` 은 스트리밍 텍스트, `input_request` 는 에이전트가 추가 입력을 요구할 때 폼을 띄우는 신호, `result` 는 최종 결과, `error` / `done` 은 종료 처리에 사용됩니다. 응답은 react-markdown + KaTeX + rehype-sanitize 로 수식 지원과 XSS 방지를 동시에 처리하여 렌더링됩니다.",
          },
          {
            type: "text",
            value: "ZIP 업로드 흐름은 \"버튼 클릭 → 매니페스트 생성\" 단계를 제거하고, 업로드 직후 precheck (의존성·진입점·필수 필드 검증) 가 자동 실행된 뒤 곧바로 AI 매니페스트 생성으로 이어지도록 단순화했습니다. 사용자가 손을 한 번 덜 움직이게 만든 작은 결정이지만, 전체 배포 흐름의 마찰을 크게 줄였습니다.",
          },
          {
            type: "text",
            value: "인증이 필요한 에이전트는 OAuth 플로우를 거칩니다. 프론트엔드에서 인증 제공자(Google · GitHub 등)의 인증 코드를 받고, 백엔드가 이를 액세스 토큰으로 교환하여 에이전트 실행 시 환경변수로 주입합니다. 사용자 인증 상태는 SessionStorage 기반 JWT 로 관리됩니다.",
          },
          {
            type: "stats",
            value: "제품적 가치 / 다음 단계",
            caption: [
              ...
              "보안 강화 (예정) — iptables 로 에이전트의 외부 API 실행을 제한하여 OAuth 코드 악용 방지",
              "수익 모델 (예정) — 구독 방식에서 토큰 사용 방식으로 전환, 매달 에이전트 비용을 개발자에게 정산",
              "멀티 플랫폼 (예정) — 웹뿐 아니라 외부 클라이언트에서도 에이전트를 호출할 수 있는 SDK 제공",
            ].join("\n"),
          },
        ],
      },
    ...
    links: {
      ...
      notice: "DooD 백엔드를 amazonsail 환경으로 이전 중이며, 시점에 따라 일부 에이전트 실행이 일시적으로 제한될 수 있습니다.",
    },
  },
  {
    id: "dictector",
